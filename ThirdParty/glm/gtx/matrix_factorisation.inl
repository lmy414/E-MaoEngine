...
namespace glm
{
	...
	GLM_FUNC_QUALIFIER void qr_decompose(mat<C, R, T, Q> const& in, mat<(C < R ? C : R), R, T, Q>& q, mat<C, (C < R ? C : R), T, Q>& r)
	{
		...
		// Source: https://en.wikipedia.org/wiki/Gram?chmidt_process
	...
	GLM_FUNC_QUALIFIER void rq_decompose(mat<C, R, T, Q> const& in, mat<(C < R ? C : R), R, T, Q>& r, mat<C, (C < R ? C : R), T, Q>& q)
	{
		...
		// QR decomposition is Gram?chmidt orthogonalization of columns of A, started from the first column.
		// RQ decomposition is Gram?chmidt orthogonalization of rows of A, started from the last row.
